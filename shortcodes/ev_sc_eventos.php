...
/**
 * Shortcode: [ev_eventos cantidad="6" area="slug-area" destacados="1"]
 * - Cabecera desde ACF: titulo_eventos / intro_eventos
 * - Cards con: thumbnail, título, meta (fecha/hora/lugar/valor), descripción y CTAs
 * - Filtros: taxonomía 'area' y meta ACF 'destacado' = 1
 * - AOS con delay progresivo ("stagger")
 * - "Ver detalle" solo si el CPT es públicamente consultable
 */
function ev_sc_eventos($atts){
  $a = shortcode_atts(['cantidad'=>6, 'area'=>'', 'destacados'=>''], $atts);

  // Cabecera del bloque (ACF en página actual u opciones)
  $titulo       = function_exists('get_field') ? get_field('titulo_eventos') : '';
  $introduccion = function_exists('get_field') ? get_field('intro_eventos')  : '';

  // Helper de truncado (si no existe ya en el tema)
  ...
  $args = [
    ...
    'tax_query'      => [],
    // Opcional: ordenar por fecha (ACF 'fecha' en formato YYYY-MM-DD o texto)
    'meta_key'       => 'fecha',
    'orderby'        => 'meta_value',
    'order'          => 'ASC',
  ];

  // Filtro por área (si existe tax 'area' también para eventos)
  if(!empty($a['area'])){
    $args['tax_query'][] = [
      'taxonomy' => 'area',
      'field'    => 'slug',
  ...
  // Saber si el CPT es públicamente consultable (tu registro tiene public=false)
  $pto  = get_post_type_object('evento');
  $can_view_single = $pto && !empty($pto->publicly_queryable);

  ob_start(); ?>
  <section class="ev-bloque ev-eventos" id="eventos" data-aos="fade-up">
    <div class="container">
      <?php if($titulo): ?>
      ...
      <div class="row g-4 row-cols-1 row-cols-sm-2 row-cols-lg-3">
        <?php
          ...
          while($q->have_posts()): $q->the_post(); $i++;
            ...
            // Descripción: ACF 'descripcion' -> excerpt -> truncado
            $desc = function_exists('get_field') ? get_field('descripcion', $id) : '';
            if(!$desc){
              $desc = has_excerpt($id) ? get_the_excerpt($id) : ev_trim_words(get_the_content(null, false, $id), 32);
            }
        ?>
          <div class="col">
            <article class="card ev-evento-card h-100" data-aos="fade-up" data-aos-delay="<?php echo esc_attr($delay); ?>">
              <?php if (has_post_thumbnail($id)): ?>
                <a href="<?php echo esc_url(get_the_post_thumbnail_url($id,'large')); ?>"
                   class="glightbox ev-evento-thumb-link" data-gallery="eventos">
                  <?php echo get_the_post_thumbnail($id, 'medium_large', ['class'=>'card-img-top ev-evento-thumb', 'alt'=>esc_attr(get_the_title())]); ?>
                </a>
              <?php endif; ?>
              ...
              <div class="card-body">
                ...
                <ul class="list-unstyled small ev-evento-meta">
...
add_shortcode('ev_eventos','ev_sc_eventos');
